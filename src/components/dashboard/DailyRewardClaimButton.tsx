...
import splitSecondsInPeriods from "~/lib/splitSecondsInPeriods";
...
const DailyRewardClaimButton = () => {
  ...
  const dailyRewardValue = useMemo(
    () =>
      dailyRewardQuery.data
        ? Number(
            (
              Number(dailyRewardQuery.data.daily_reward) +
              Number(dailyRewardQuery.data.pending_referral_rewards)
            ).toFixed(2),
          )
        : 0,
    [dailyRewardQuery.data],
  ...
  const handleClaim = useCallback(async () => {
    ...
    try {
      await claim();
      await Promise.all([balancesQuery.refetch(), dailyRewardQuery.refetch()]);
  ...
  useEffect(() => {
    ...
    const formattedDate = dailyRewardQuery.data.reset_at
      .replace(" ", "T")
      .replace(" UTC", "Z");
    ...
    const updateCountdown = (target: number) => {
      ...
      if (difference > 0) {
        const { hours, minutes, seconds } = splitSecondsInPeriods(
  ...
  const isLoading =
    isClaiming || dailyRewardQuery.isPending || dailyRewardQuery.isFetching;
